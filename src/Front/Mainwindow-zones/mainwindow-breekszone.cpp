...
void MainWindow::deleteBreeksZone(int zoneIndex) {
  QUrl url = QUrl(Network::serverUrl + Network::deleteBreeksLineUrl + "/" +
                  QString::number(arrBreeksZones_[zoneIndex].idOnServer));
    ...
    if (dayIndex < DAYS_COUNT - 1) {
      ...
      if (!zone->arrBreeks[dayIndex + 1]->isEnabled()) {
        ...
        if (zone->arrBreeks[dayIndex + 1]->getEmojiNum() != zone->arrBreeks[dayIndex]->getEmojiNum()) {
          ...
          zone->arrBreeks[dayIndex + 1]->connectToQml(zone->arrBreeks[dayIndex + 1]->getEmojiNum(),
                                                      zone->arrBreeks[dayIndex + 1]->getColorState());
          QThread::msleep(100);
        }
