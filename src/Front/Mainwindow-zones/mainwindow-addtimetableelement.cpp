...
void MainWindow::setStyleAddTimetableElementForm() {
  ui->addTimetableElementGb->setStyleSheet("QGroupBox{background: #cce4ff; border: 1px solid #C6C6C6; "
                                           "border-radius: 18px;}");

  indexCurrentTag_ = 0;
  ui->timetableElementTagColor->setStyleSheet("QPushButton{background: " + tag::ARR_COLORS[indexCurrentTag_] +
                                              "; border-radius: 6px;}");
  ...
  ui->timetableElementDescription->setFont(QFont(QStringLiteral("Helvetica"), 12));
  ...
  ui->mnTimetableElemnetButton->setFont(QFont(QStringLiteral("Helvetica")));
  ui->tuTimetableElemnetButton->setStyleSheet(dayButtonStyleSheet);
  ui->tuTimetableElemnetButton->setFont(QFont(QStringLiteral("Helvetica")));
  ui->weTimetableElemnetButton->setStyleSheet(dayButtonStyleSheet);
  ui->weTimetableElemnetButton->setFont(QFont(QStringLiteral("Helvetica")));
  ui->thTimetableElemnetButton->setStyleSheet(dayButtonStyleSheet);
  ui->thTimetableElemnetButton->setFont(QFont(QStringLiteral("Helvetica")));
  ui->frTimetableElemnetButton->setStyleSheet(dayButtonStyleSheet);
  ui->frTimetableElemnetButton->setFont(QFont(QStringLiteral("Helvetica")));
  ui->stTimetableElemnetButton->setStyleSheet(dayButtonStyleSheet);
  ui->stTimetableElemnetButton->setFont(QFont(QStringLiteral("Helvetica")));
  ...
  ui->addTimetableElementButton->setStyleSheet("QPushButton{background: #b9de6f; border-radius: 20px; border: 0.2px "
                                               "solid #d7eba9; color: #ffffff; font-size: 12pt; font-weight: bold;} "
                                               "QPushButton:hover{background: #d2e9a0; border-radius: 20px; color: "
                                               "#FFFFFF; font-size: 12pt; font-weight: bold;}");
...
void MainWindow::on_addTimetableElementButton_clicked() {
  ...
  if (!tmpCond || ui->timetableElementDescription->toPlainText() == QLatin1String("")) {
    return;
  }
  ...
  if ((ui->timetableElementTimeStart->time() < ui->timetableElementTimeEnd->time()) ||
      (ui->timetableElementTimeEnd->time().toString() == QLatin1String("00:00:00"))) {
    ...
    newElement.timeStart = ui->timetableElementTimeStart->time().toString();
    newElement.timeEnd = QStringLiteral("00:00:00");
  } else {
    // reverse time
