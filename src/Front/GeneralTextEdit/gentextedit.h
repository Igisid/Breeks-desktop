...
#include "Front/Dictionary/russiandictionary.h"
...
public:
  ...
  // in ...-get-set.cpp
  void fillCharStyleVector(int cursorPos, int count, const charStyle_t &ch);
...
private:
  std::unique_ptr<UndoRedoText> undoRedoBuffer_;
  void setCommandInfo(commandInfo_t &command, const enum command commandName, const int pos, const QString &text,
                      int lenght = -1);
  static inline RussianDictionary rusDic_;
  std::unique_ptr<QTimer> timer_;
  std::unique_ptr<QTimer> requestTimer_;
  ...
  const QString dashSign_ = QStringLiteral("—");
  const QString pointSign_ = QStringLiteral("•");
  const QString minusSign_ = QStringLiteral("-");
  const QString warrningSign_ = QStringLiteral("⚠");
  ...
  void deleteSmth(const Qt::KeyboardModifiers kmModifiers, const QTextCursor::MoveOperation whereMove, int &cursorPos,
                  const int blindSpot, const int a = 0);
  ...
  // if cursor in the middle of item and we are going to push Bs/del we should
  // delete full item
  void detailsCheckItemPosInDeleting(int &cursorPos, const bool isBS, Qt::KeyboardModifiers &mod);

  // if we write smth in the middle of item, we won't have the item it will
  // become a regular text
  ...
  static void detailsSetFormatFields(QTextCharFormat &fmt, const charStyle_t &ch);
  ...
  static void detailsColorText(QTextCursor c, const QString &color);
  ...
  static bool detailsIsLetter(const QChar ch);
